...
class MiniCardGridView: UICollectionViewCell {
   ...
   @objc func chatButtonTapped(_ sender: UIButton) {
      ...
      channelStream.createChannel(channelId: channelId, sender: currentUserNickName, senderJobTitle: currentUserOccupation, senderProfileImage: currentUserProfileImage, senderEmail: currentUserEmail, receiver: miniCardView.userNameLabel.text ?? "", receiverJobTitle: miniCardView.userPartLabel.text ?? "", receiverProfileImage: receiverurl, receiverEmail: receiverEmail) {error in
         if let error = error {
